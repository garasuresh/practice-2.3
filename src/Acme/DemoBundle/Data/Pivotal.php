...
//        $weatherData = json_decode(json_encode((array)$feed),1);
    ...
    // -----
    // Get a list of stories from a project, optionaly filter
    public function getStory($storyId, $filter = '') {
        ...
        // Make the fields safe
        $filter = escapeshellcmd($filter);
        $project = escapeshellcmd($this->project);

        // Request the stories
        $cmd = "curl -H \"X-TrackerToken: {$this->token}\" "
            . "-X GET "
            . "https://www.pivotaltracker.com/services/v3/projects/$project/stories/$storyId";
        // Add the filter, if it was specified
        if ($filter != '') $cmd .= "?filter=$filter";
        $xml = shell_exec($cmd);
